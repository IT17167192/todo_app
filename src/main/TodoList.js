        ...
        return (
            <Card ref={props.reference + item.key} bg="dark" text='white' key={item.key} className="mb-1">
                <Card.Body>
                    ...
                    <Row>
                            <Col className="text-left ml-3" style={{color: "#2196F3"}}>{dayjs(item.key).fromNow()}</Col>
                            <Col>On {dayjs(item.date).format("dddd, MMMM D YYYY")} at {dayjs(item.time).format("h:mm a")}</Col>
                            <Col className="text-right"><Tooltip title={item.completed ? "" : "Edit"}
                                                                 placement="bottom-end"><Button
                                variant="info" style={{cursor: item.completed ? "no-drop" : ""}}
                                disabled={item.completed ? true : false}
                                onClick={() => props.editTaskById(item.key)}><EditTwoToneIcon/></Button></Tooltip>{' '}<Tooltip
